...
class Baseline:
    ...
    def __init__(self, elevations, distances, precise : bool = True):
        if not precise   :
            self.points = self._als_baseline(elevations, lam=1500, p=0.01)    
    ...
    def _optimize_als_params(self, elevations, lam_range=(1, 1e6), p_range=(0.001, 0.1)):
    ...
    def _calculate_baseline_precise(self, elevations, distances):
        ...
        baseline = self._als_baseline(elevations, lam=optimal_lam, p=0.01)
